...
def train(
    ...
    and sample noise to generate a fake data batch using the generator.
    Since the generator is needed when calculating the discriminator's loss,
    you will need to call .detach() on the generator result to ensure that only the discriminator is updated!
    Calculate the discriminator output for real and fake data, and use the output to compute
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            G.zero_grad()
            noise = sample_noise(batch_size, noise_size).to(device).view(batch_size, noise_size, 1, 1)  # Преобразуем noise
            fake_images = G(noise)
